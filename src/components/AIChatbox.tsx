...
export const AIChatbox: React.FC = () => {
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-slate-900/80 shadow-2xl backdrop-blur-sm overflow-hidden">
      <div className="flex items-center justify-between border-b border-white/10 px-4 py-3 bg-slate-950/50">
        <div className="flex items-center gap-2.5">
          <div className="rounded-xl bg-teal-500/15 p-2 text-teal-300">
            <MessageCircle className="w-4 h-4" />
          </div>
          <div>
            <p className="text-sm font-bold text-white">AI Health Assistant</p>
            <p className="text-[10px] uppercase tracking-[0.2em] text-slate-400">Clinical guidance</p>
          </div>
        </div>
        <div className="flex items-center gap-1 rounded-full bg-teal-500/10 px-2 py-1 text-[10px] text-teal-300">
      ...
      <div className="p-4 space-y-3">
        <div className="flex flex-wrap gap-2">
          {starterPrompts.map((prompt) => (
            <button
              ...
              className="rounded-full border border-teal-500/20 bg-teal-500/10 px-2.5 py-1.5 text-[11px] text-teal-200 transition hover:bg-teal-500/20"
            >
              {prompt}
            </button>
          ))}
        ...
        <div className="max-h-72 overflow-y-auto space-y-3 rounded-2xl bg-slate-950/40 p-3">
          {messages.map((message, index) => (
            <div
              key={`${message.role}-${index}`}
              className={`max-w-[90%] rounded-2xl px-3 py-2 text-sm leading-relaxed ${
                message.role === 'user'
                  ? 'ml-auto bg-teal-500 text-slate-950 font-medium'
        ...
        {(lastSuggestions.length > 0 || lastMedicines.length > 0) && (
          <div className="space-y-3 rounded-2xl border border-teal-500/20 bg-teal-500/5 p-3">
            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-teal-300">
            ...
            {lastMedicines.length > 0 && (
              <div className="space-y-2 pt-2">
                ...
                {lastMedicines.map((medicine) => (
                  <div key={medicine.name} className="rounded-xl border border-white/10 bg-slate-900/70 p-2.5">
                    <p className="font-semibold text-white">{medicine.name}</p>
                    <p className="text-xs text-slate-300">{medicine.reason}</p>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="space-y-3">
          <div className="rounded-2xl border border-white/10 bg-slate-950/50 p-3">
            <label className="mb-2 block text-[10px] uppercase tracking-[0.2em] text-slate-400">Message</label>
            <textarea
              value={draft}
              onChange={(e) => setDraft(e.target.value)}
              placeholder="Describe your symptoms or ask for medicine suggestions..."
              className="min-h-[72px] w-full resize-none rounded-xl border border-white/10 bg-slate-900 px-3 py-2 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-teal-500/30"
            />
          </div>
