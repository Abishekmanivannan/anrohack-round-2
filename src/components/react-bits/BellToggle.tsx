...
const ringKeyframes = (from: number, amplitude: number, passes: number, decay: number) => {
...
const liveAngle = (el: HTMLElement) => {
...
export default function BellToggle({
  ...
  const clip = useTransform([t, wOff, wOn], (latest: any[]) => {
    const [v, a, b] = latest as [number, number, number];
  ...
  useLayoutEffect(() => {
    ...
    spring.current?.stop();
    if (pointer) spring.current = animate(t as any, (on ? 1 : 0) as any, { ...SPRING_UI, bounce: revealBounce } as any);
    else if ((t as any).jump) (t as any).jump(on ? 1 : 0);
    else t.set(on ? 1 : 0);
