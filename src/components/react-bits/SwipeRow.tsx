...
const velocityOf = (hist: Array<[number, number]>) => {
...
const watchWindow = (live: React.MutableRefObject<any>) => {
...
export default function SwipeRow({
  ...
  const foldTimer = useRef<any>(undefined);
  ...
  const shift = useTransform([exposed, spread], ([e, p]) => (p as number) * Math.max(0, (e as number) - A));
  const blockXf = useTransform(shift, (v) => `translateX(${s * (v as number)}px)`);
  const glyphXf = useTransform(
    [shift, landed],
    ([v, l]) => `translateX(${-s * (l as number) * ((v as number) - (w.current - A) / 2)}px)`
  );
  ...
  const settle = (target: number, v = 0) => {
    ...
    if (reduce) {
      animate(x as any, (s * target) as any, { duration: 0.2, ease: EASE_OUT } as any);
      return;
    }
  ...
  const setSpread = (on: boolean) => {
    ...
    else animate(spread as any, (on ? 1 : 0) as any, SPRING_UI as any);
    if (on && primary) {
      setSay(`Release to ${primary.label}`);
      if (haptic && grip.current?.touch) (navigator as any).vibrate?.(8);
    }
  };
  ...
  const commit = (a: SwipeAction, viaKey?: boolean, v = 0) => {
    ...
    if (viaKey || reduce) {
      ...
      if (viaKey) {
        x.set(s * w.current);
        fold();
      } else animate(x as any, (s * w.current) as any, { duration: 0.2, ease: EASE_OUT } as any).then(fold);
      return;
    }
    if (leap) {
      if (spread.get() < 1) animate(spread as any, 1 as any, SPRING_UI as any);
      animate(landed as any, 1 as any, SPRING_UI as any);
    }
    animate(x as any, (s * w.current) as any, { ...SPRING_UI, velocity: s * v } as any).then(fold);
  };
        ...
        >
          ...
          {primary ? (
            <motion.div className="swipe-row__block" style={{ transform: blockXf }}>
              ...
              >
                <span className="swipe-row__glyph">
                  <span className="swipe-row__icon">{primary.icon ?? <Trash2 className="w-5 h-5" />}</span>
