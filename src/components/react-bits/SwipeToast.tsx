...
export default function SwipeToast({
  ...
  const reason = useRef<string>('timeout');
  ...
  const swipeOut = (dy: number, v: number) => {
    ...
    if (!reduce && cardRef.current) {
      animate(y as any, (dy + cardRef.current.offsetHeight) as any, { type: 'spring', duration: 0.3, bounce: 0, velocity: v * 1000 } as any);
    }
    animate(fade as any, 0 as any, { duration: 0.2, ease: EASE_OUT } as any).then(() => finish('swipe'));
  };
    ...
    if (d.moved) {
      animate(
        ...
        (reduce
          ...
          : { type: 'spring', duration: 0.5, bounce: settleBounce, velocity: v * 1000 }) as any
      );
    }
